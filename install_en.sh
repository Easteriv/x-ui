...
install_x-ui() {
    ...
    if [ $# == 0 ]; then
        ...
        wget -N --no-check-certificate -O /usr/local/x-ui-linux-${arch}-english.tar.gz https://github.com/Easteriv/x-ui/releases/download/${last_version}/x-ui-linux-${arch}-english.tar.gz
        if [[ $? -ne 0 ]]; then
        ...
        last_version=$1
        url="https://github.com/Easteriv/x-ui/releases/download/${last_version}/x-ui-linux-${arch}-english.tar.gz"
    ...
    wget --no-check-certificate -O /usr/bin/x-ui https://raw.githubusercontent.com/Easteriv/x-ui/main/x-ui_en.sh
